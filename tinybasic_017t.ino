...
//
// v0.17: 2018-5-31 (Harm)
//      added MIN A,B and MAX A,B that put the min or max of A and B in A
//      B can also be a constant (i.e. MIN K,123).
//      changed BELL character from '\b' to '\a', renamed CMDS to HELP. fixed error 
//      with file_io (location of commands in enum). re ordered commands to get most used 
//      on top (abbreviate shorter)
//      fixed bug in DELAY to allow multiple statemens on one line
//      changed serial buffer size in HardwareSerial.cpp from 64 to 32 to gain 
//      RAM for programs in UNO.This must be applied in the cpp files since naming is different
//      over Arduino toochain versions. v0.17 is a renaming of dev. version 0.16.3
//
// v0.16: 2018-5-24 (Harm)
//      add keyword CMDS that lists all commands in this version of
//      tinybasic. Fixed backspace issue.
//      add keyword VARS that shows current values of all 26 variables
//      Removed LOAD/SAVE/FILES/CHAIN in basic ARDUINO - No FILE_IO config
//      Added MILS X that gives miliseconds from last MILIS, or start arduino (max 32k).
//
// v0.15: 2018-2-16 (Harm)
//      add hardware supported SERVO pin,angle and NOSERVO pin.
//      UNO: 2 servo's only for pin 9 and 10. MEGA 3 servo's pin 11,12,13 
//
// v0.14: 2016-11-16 (Harm)
//      several patches (Brian O'Dell) in input statement
//      Modifie input command to accept an expression using getn()
//      Syntax is "input x" where x is any variable
//      Modified kRamSize from 1160 to 1199 to get rid of problems with
//      functions (repeated) stopping keyboard input.
//
// v0.13a: 2015-02-09 (Harm)
//      Abbreviate keywords with '.' to store more efficient. i.e. 'PRINT' becomes 'P.'
//      The programmer has to evaluate if sufficient characters are provided for the 
//      abbreviation to translate right. i.e. PO. = POK. = POKE, R. = RUN
//      Can be confusing but uses memory more efficient.
//
// v0.13: 2013-03-04
...
// on Arduino boards we can add servo's. Angle 0-180 degrees. Adding servo's 
// disables AWRITE functionality as follows
// UNO/NANO/MINI: no awrite on pin 9 and 10
...
#define kRamSize  (RAMEND - 1199 + 64 - kRamFileIO - kRamTones) //changed UART buffers from 64 to 32 @ Harm
#endif
...
#define LF    0x0a
#define TAB	'\t'
#define BELL	'\a' //'\b' //was b, but ascii for bell is a
#define SPACE   ' '
#define SQUOTE  '\''
#define DQUOTE  '\"'
#define CTRLC	0x03
...
#endif
#endif
  KW_HELP, /* lists all commands in this version*/
  KW_VARS,
...
static void scantable(const unsigned char *table)
{
  ...
  while(1)
  {
    ...
    else
    {
      ...
      // @harm@ here we check if keywords are abbreviated with a period '.'
      else if(txtpos[i] == '.')
      {
        txtpos += i+1;  // Advance the pointer to following the keyword
        ignore_blanks();
...
/***************************************************************************/
// list commands in keyword tables
void printCMDs(const unsigned char *msg)
{
  unsigned char c;
  while( pgm_read_byte( msg ) != 0 ) {
    c = (pgm_read_byte( msg++));
...
void printVARs()
{
    short int a;
    unsigned char p; 
    //ripple through all vars
    for (p='A';p<='Z';p++) {
      a = ((short int *)variables_begin)[p - 'A'];
      outchar (p);
      outchar ('=');
      printnum(a);
      outchar (' ');
...
minmax:
  {
    // max a,b : determine the max value of a and b and put in a
    // or de min value 
    unsigned char vara;
    short int a;
    ...
    if (isMax == true) {
      // here we adapt a if it is smaller than b  
      if (((short int *)variables_begin)[vara - 'A'] < a)   
        ((short int *)variables_begin)[vara - 'A'] = a;
    }
    else {
      // here we adapt a if it is larger than b
      if (((short int *)variables_begin)[vara - 'A'] > a)   
        ((short int *)variables_begin)[vara - 'A'] = a;
    }      
...
commands:
  // list all commands in this version of tinybasic
  printCMDs(keywords);
  printCMDs(func_tab);
  printCMDs(to_tab);
  printCMDs(step_tab);
  printCMDs(relop_tab);
  printCMDs(highlow_tab);
...
/*************************************************/
// Harm: not needed in arduino without SD
files:
...
servo_off:
{
    ...
    //can we free timer 1 again after all servo's detach, and make the timer work for analog write
#if defined(__AVR_ATmega1280__) || defined(__AVR_ATmega2560__) // Arduino Mega
    if ( !servoA.attached() && !servoB.attached() && !servoC.attached() ) {
#else //UNO
    if ( !servoA.attached() && !servoB.attached() ) {
#endif
    TCCR1A = _BV(WGM10);              // brute force back to default, since we know no interrupts run
    TCCR1B = _BV(CS11) | _BV(CS10);   // we have disabled them when starting the servo
    }  
  }
  goto run_next_statement;
